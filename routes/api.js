...
router.get('/ig-dl', async(req, res) => {
    ...
    if (!url) {
        ...
        anu = await axios.get('https://api.lolhuman.xyz/api/instagram?apikey=MYNAMEISRAMLAN&url='+ url +'')
        let { result } = anu.data
        res.send({
              status: 200,
              creator: 'Ramlan ID',
...
router.get('/igstalk', async(req, res) => {
    ...
    if (!username) {
        ...
        anu = await axios.get('https://api.lolhuman.xyz/api/stalkig/' + username + '?apikey=MYNAMEISRAMLAN')
        let { result } = anu.data
        res.send({
             status: 200,
             creator: 'Ramlan ID',
...
router.get('/nickpubg', async(req, res) => {
    ...
    if (!id) return res.json({ status: false, creator: 'Ramlan ID', message: 'Masukan id' })
...
router.get('/nickml', async(req, res) => {
    ...
    if (!id) return res.json({ status: false, creator: 'Ramlan ID', message: 'Masukan id' })
    if (!server) return res.json({ status: false, creator: 'Ramlan ID', message: 'Masukan server' })
...
router.get('/nickdomino', async(req, res) => {
    var id = req.query.id
    var apikey = req.query.apikey
    var isPremium = await premium.checkPremiumUser(apikey);
    ...
    if (!isPremium) {
        ...
        anu = await axios.get('https://api.lolhuman.xyz/api/higghdomino/' + id + '?apikey=MYNAMEISRAMLAN')
        let { result } = anu.data
        res.send({
            status: 200,
            creator: 'Ramlan ID',
...
router.get('/tiktok', async(req, res) => {
    ...
    res.status(500).send({
    status: 500,
    message: 'masukan link tiktok nya pren'
    })
    } else {
        anu = await axios.get('https://api.lolhuman.xyz/api/tiktok?apikey=MYNAMEISRAMLAN&url='+ url +'')
        let { result } = anu.data
        res.json({
            status: 200,
            creator: 'Ramlan ID',
...
router.get('/random/asupan', async(req, res) => {
    ...
    var result = await axios.get(`https://www.instagram.com/${username}/channel/?__a=1`, { headers: { cookie: "ig_nrcb=1; mid=YSuOOQALAAGkt2WPJrNcElOGLEtM; ig_did=0FCEAEE0-4EC5-4A70-8E58-014C4AE4DF4D; csrftoken=SosDB0arfALrveBISdjHWNSlFAw6a8RC; ds_user_id=44290388479; sessionid=44290388479:6MkMXJ3mmv3Q9I:23; rur=\"ATN\\05448544354370\\0541658635689:01f7b64bea46a6d32a9b8914573063423b50a303e6c1f6bb5bdc37ea132488c64e2c4171\"" } })
